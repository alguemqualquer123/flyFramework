...
import { scopeCss, generateUtilities, hashFile, collectStyles } from "../src/core/css.ts";
...
test("Form: SSR emite data-fly-form + data-fly-action (sem avaliar a função)", async () => {
  const c = compile(`<script>let t = $state("x")</script><Form action={addTodo}><input name="text"/><button>Enviar</button></Form>`, { file: "/x/form.fly" });
  const o = await c.render({ params: {}, request: new Request("http://x/"), url: new URL("http://x/"), cache: {} });
...
test("islands: SSR emite template data-fly-island; client usa mountIsland", async () => {
  const c = compile(`<div client:load>oi</div><section client:visible>v</section>`, { file: "/x/island.fly" });
  const o = await c.render({ params: {}, request: new Request("http://x/"), url: new URL("http://x/"), cache: {} });
...
test("Suspense: SSR renderiza conteúdo; client usa mountSuspense", async () => {
  const c = compile(`<Suspense fallback="<p>carregando</p>"><div>real</div></Suspense>`, { file: "/x/susp.fly" });
  const o = await c.render({ params: {}, request: new Request("http://x/"), url: new URL("http://x/"), cache: {} });
...
test("revalidatePath normaliza e prefixa a chave de rota", () => {
  routeCache.set("route:/blog/hello", { body: "x", status: 200, headers: {} }, 60);
...
test("buildProd escreve assets e SEO em .fly-out", async () => {
  const scan = scanApp(appDir);
  const b = createBuild(scan, { appDir, siteUrl: "https://fly.dev", outDir: ".fly-test-out" });
  await b.buildProd();
  ...
  assert.ok(existsSync(join(process.cwd(), ".fly-test-out", "_fly", "client.js")));
  void readFileSync;
});
...
test("compile expõe <style> e aplica data-fly-scope no SSR/client", async () => {
  ...
  const o = await c.render({ params: {}, request: new Request("http://x/"), url: new URL("http://x/"), cache: {} });
  assert.match(o.html, new RegExp(`data-fly-scope="${scope}"`), "SSR aplica data-fly-scope no elemento");
  assert.match(c.clientCode, new RegExp(`data-fly-scope.*${scope}`), "client aplica data-fly-scope");
});

test("scopeCss prefixa seletores e preserva @media e @keyframes", () => {
  const scope = `[data-fly-scope="${hashFile("/x.fly")}"]`;
  const out = scopeCss(".a{color:red}.b:hover{color:blue}@media (min-width:640px){.c{display:flex}}@keyframes spin{from{transform:rotate(0)}to{transform:rotate(1turn)}}", "/x.fly");
  assert.ok(out.startsWith(scope), "prefixa a primeira regra");
  assert.match(out, /\[data-fly-scope[^\]]*\] \.b:hover/, "prefixa segunda regra");
  assert.match(out, /@media \(min-width:640px\)\{\[data-fly-scope[^\]]*\] \.c/, "escopa dentro de @media");
  assert.match(out, /@keyframes spin\{from\{transform:rotate\(0\)/, "keyframes intacto");
});
...
test("pipeline serve /__fly/styles.css com utilities + estilos escopados", async () => {
  ...
  const res = await pipeline.handle(new Request("http://localhost/__fly/styles.css"));
...
test("buildProd escreve _fly/styles.css", async () => {
  ...
  const b = createBuild(scan, { appDir, siteUrl: "https://fly.dev", outDir: ".fly-test-out" });
  await b.buildProd();
  assert.ok(existsSync(join(process.cwd(), ".fly-test-out", "_fly", "styles.css")), "styles.css gerado no build");
  const css = readFileSync(join(process.cwd(), ".fly-test-out", "_fly", "styles.css"), "utf8");
  assert.match(css, /\.flex\{display:flex\}/, "utilities no stylesheet de produção");
});

// ---------- Drag & drop ----------
test("DnD: on:dragstart / on:drop geram addEventListener no client", () => {
  const c = compile(`<div on:dragstart={start} on:drop={drop} on:dragover={over}><p>x</p></div>`, { file: "/x/dnd.fly" });
  assert.match(c.clientCode, /addEventListener\("dragstart"/, "dragstart listener");
  assert.match(c.clientCode, /addEventListener\("drop"/, "drop listener");
  assert.match(c.clientCode, /addEventListener\("dragover"/, "dragover listener");
});
...
test("DnD: moveItem reordena lista de forma imutável", async () => {
  ...
  assert.deepEqual(list, ["a", "b", "c", "d"], "lista original intacta (imutável)");
  assert.deepEqual(moveItem(list, 3, 0), ["d", "a", "b", "c"], "move d para o início");
...
test("generateTypes produz fly.d.ts importável com rotas/actions/tipos", () => {
  ...
  assert.match(dts, /import type \{ Loader, ServerAction/, "re-exporta Loader/ServerAction");
