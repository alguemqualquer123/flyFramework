...
exports.FlyHoverProvider = void 0;
const vscode = __importStar(require("vscode"));
const FLY_API_HOVERS = [
    {
        pattern: /\$state\s*\(/,
        content: [
...
const TEMPLATE_EXPRESSION_HOVERS = [
    {
        pattern: /\{data\.\w+\}/,
        content: [
...
class FlyHoverProvider {
    provideHover(document, position, _token) {
        ...
        if (hoverFromContext)
            return hoverFromContext;
        return null;
    }
    getContextHover(document, position) {
        const line = document.lineAt(position.line).text;
        ...
        if (stateVarMatch) {
            ...
            if (initialValue) {
                if (/^\d+(\.\d+)?$/.test(initialValue))
                    ...
                    typeStr = "string";
                else if (/^(true|false)$/.test(initialValue))
                    typeStr = "boolean";
                else if (/^\{/.test(initialValue))
                    typeStr = "object";
                else if (/^\[/.test(initialValue))
        ...
        if (loaderMatch) {
            const params = loaderMatch[2].split(",").map((p) => p.trim()).filter(Boolean);
            const md = new vscode.MarkdownString([
        ...
        if (apiMatch) {
            ...
            const params = apiMatch[3].split(",").map((p) => p.trim()).filter(Boolean);
            const md = new vscode.MarkdownString([
...
exports.FlyHoverProvider = FlyHoverProvider;
//# sourceMappingURL=flyHover.js.map