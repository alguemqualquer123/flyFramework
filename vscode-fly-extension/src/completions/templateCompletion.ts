...
const FLY_DIRECTIVES: { label: string; snippet: string; doc: string }[] = [
  {
    label: "if",
...
const FLY_EVENTS: { label: string; snippet: string; doc: string }[] = [
  {
    label: "on:click",
...
export class FlyTemplateCompletionProvider implements vscode.CompletionItemProvider {
  provideCompletionItems(
    ...
    if (this.isInsideStyleBlock(textBefore)) return [];
    ...
    if (inTag) {
      ...
      const htmlAttrs: { attr: string; doc: string }[] = [
    ...
    if (this.isInsideInterpolation(lineText, position.character)) {
      ...
      const builtins: { name: string; doc: string }[] = [
  ...
  private isInsideStyleBlock(text: string): boolean {
  ...
  private isInsideInterpolation(line: string, charPos: number): boolean {
    let depth = 0;
