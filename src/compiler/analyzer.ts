...
function findAllExportedFunctions(
  script: string,
...
export function analyzeScript(script: string): Analyzed {
  ...
  const reDerived =
    /(?:let|const|var)\s+([A-Za-z_$][\w$]*)\s*=\s*\$derived\s*\(/g;
  const reStore =
    /(?:let|const|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:writable|readable)\s*\(/g;
  let m: RegExpExecArray | null;
  ...
  const directive: "client" | "server" | null = directiveMatch
    ? (directiveMatch[1] as any)
    : null;
  ...
  const generateStaticParams =
    findExported(script, "generateStaticParams") ?? undefined;
  ...
  if (/export\s+const\s+prerender\s*=\s*true/.test(script))
