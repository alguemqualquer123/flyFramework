...
export type Segment =
  { kind: "text"; value: string } | { kind: "expr"; value: string };
  ...
  function parseChildren(stopTag: string | null): Node[] {
    ...
    while (i < input.length) {
      if (input[i] === "<") {
        ...
        const attrContent =
          spaceIdx === -1 ? "" : tagContent.slice(spaceIdx + 1);
        ...
        const restAttrs = attrs.filter(
          (a) => a.name !== "if" && a.name !== "for",
        );
        ...
        if (tag.toLowerCase() === "suspense") {
          ...
          out.push({
            type: "suspense",
            attrs: restAttrs.filter((a) => !a.name.startsWith("client:")),
            children,
          });
          continue;
        }
        ...
        if (tag === "Await") {
          const children = selfClosing ? [] : parseChildren(tag);
          const promAttr = attrs.find((a) => a.name === "promise");
          const letAttr = attrs.find(
            (a) => a.name === "let" || a.name === "as",
          );
          const fbAttr = attrs.find((a) => a.name === "fallback");
          const errAttr = attrs.find(
            (a) => a.name === "error" || a.name === "catch",
          );
          out.push({
            type: "await",
            promise: promAttr
              ? parseExpr(promAttr.value)
              : "Promise.resolve(null)",
            letName: letAttr ? parseExpr(letAttr.value) : "data",
            fallback: fbAttr
              ? fbAttr.value.replace(/^[{\"\']|[}\"\']$/g, "")
              : "Carregando…",
            errorFallback: errAttr
              ? errAttr.value.replace(/^[{\"\']|[}\"\']$/g, "")
              : "",
            children,
          });
          continue;
        ...
        const islandAttr = attrs.find((a) =>
          /^client:(load|visible|idle)$/.test(a.name),
        );
        const children = selfClosing ? [] : parseChildren(tag);
        ...
        if (text.trim())
          out.push({ type: "text", segments: splitSegments(text) });
      }
    }
    return out;
  }
