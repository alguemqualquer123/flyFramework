...
function resolveComp(
  tag: string,
  attrs: Attr[],
): { tag: string; attrs: Attr[] } {
  if (tag === "Link") {
    return {
      tag: "a",
      attrs: [{ name: "data-fly-link", value: "", dynamic: false }, ...attrs],
    };
  }
  if (tag === "Image") {
    return {
      ...
      attrs: [
        ...attrs,
        { name: "loading", value: "lazy", dynamic: false },
        { name: "decoding", value: "async", dynamic: false },
      ],
    };
  }
  if (tag === "Form") {
    ...
    const outAttrs: Attr[] = [
      { name: "data-fly-form", value: "true", dynamic: false },
    ];
    for (const a of attrs) {
      if (a.name === "action") {
        ...
        const val = a.value.includes("{")
          ? a.value.replace(/[{}]/g, "").trim()
          : a.value;
...
function escapeFb(s: string): string {
...
function srvNodes(nodes: Node[]): string {
  ...
  for (const n of nodes) {
    if (n.type === "text") code += srvSegments(n.segments);
      ...
      if (n.name)
        code += `__html += ((slots && slots[${JSON.stringify(n.name)}]) ?? "");\n`;
      ...
      const id = `a${_awaitSeq++}`;
      const varName = /^[A-Za-z_$][\w$]*$/.test(n.letName) ? n.letName : "data";
      const promiseExpr = rwReads(n.promise, _ssrNames);
      const fb = n.fallback.includes("<") ? n.fallback : escapeFb(n.fallback);
      const errFb = n.errorFallback
        ? n.errorFallback.includes("<")
          ? n.errorFallback
          : escapeFb(n.errorFallback)
        : fb;
      code += `__html += '<template data-fly-await="${id}">' + ${JSON.stringify(fb)} + '</template>';\n`;
      const childSrc = srvNodes(n.children).split("__html").join(`__h_${id}`);
      code += `__fly_deferred.push((async () => { try { const ${varName} = await (${promiseExpr}); let __h_${id} = "";\n${childSrc}\nreturn { id: ${JSON.stringify(id)}, html: __h_${id} }; } catch (__e) { return { id: ${JSON.stringify(id)}, html: ${JSON.stringify(errFb)}, error: String(__e && __e.message ? __e.message : __e) }; } })());\n`;
    } else if (n.type === "if") {
      const el: Node = {
        type: "element",
        tag: n.tag,
      ...
      const iter = n.index
        ? `[${n.item}, ${n.index}] of (${n.of}).entries()`
        : `${n.item} of (${n.of})`;
      const el: Node = {
        type: "element",
        tag: n.tag,
...
export function generateServer(
  ...
  const hasUserT =
    /(?:let|const|var)\s+t\b/.test(serverScript) || _ssrNames.includes("t");
  const hasUserLocale =
    /(?:let|const|var)\s+locale\b/.test(serverScript) ||
    _ssrNames.includes("locale");
  const alias =
    (hasUserT ? "" : `const t = __fly_t;\n`) +
    (hasUserLocale ? "" : `const locale = __fly_locale;\n`);
  const body =
    `const { params, request, slotContent, slots, cache, url } = ctx;\n` +
    `const __fly_locale = ctx.locale; const __fly_t = ctx.t ?? ((k) => k);\n` +
    ...
    "\n" +
    `let data = {};\n` +
    `if (typeof loader !== "undefined") { data = await loader({ params, request, cache, url, locale: __fly_locale, t: __fly_t }); }\n` +
    `let __html = "";\n` +
    `let __fly_deferred = [];\n` +
    srvNodes(ast) +
    `return { html: __html, data, deferred: __fly_deferred, meta: (typeof meta !== "undefined" ? meta : null), middleware: (typeof middleware !== "undefined" ? middleware : null), api: {\n` +
...
// ---------- CLIENT ----------
function cliSegments(
  segs: { kind: string; value: string }[],
  state: string[],
  derived: string[],
): string[] {
  const out: string[] = [];
  ...
  for (const s of segs) {
    if (s.kind === "text") buf += s.value;
      ...
      flush();
      out.push(`text(() => ${rw(s.value, state, derived)})`);
    }
  }
  flush();
  return out;
  ...
  return parts
    .map((p) =>
      p.kind === "text"
        ...
        : `String(${rw(p.value, state, derived)})`,
    )
    .join(" + ");
}
...
function cliNodes(nodes: Node[], state: string[], derived: string[]): string[] {
  ...
  for (const n of nodes) {
    if (n.type === "text") out.push(...cliSegments(n.segments, state, derived));
      ...
      const varName = /^[A-Za-z_$][\w$]*$/.test(n.letName) ? n.letName : "data";
      const kids = cliNodes(n.children, state, derived).join(", ");
      out.push(
        `mountAwait(() => (${rw(n.promise, state, derived)}), (${varName}) => [${kids}], ${JSON.stringify(n.fallback)})`,
      );
    } else if (n.type === "if") {
      const el: Node = {
        type: "element",
      ...
      out.push(
        `hIf(() => (${rw(n.test, state, derived)}), () => [${cliElement(el, state, derived)}])`,
      );
    } else if (n.type === "for") {
      ...
      out.push(
        `hFor(() => (${rw(n.of, state, derived)}), (${args}) => [${cliElement(el, state, derived)}])`,
      );
...
function cliElement(
  ...
  for (const a of rAttrs) {
    if (a.name.startsWith("transition:")) {
      ...
      dyn.push(
        `__el.addEventListener(${JSON.stringify(evt)}, () => { ${rw(handler, state, derived)} });`,
      );
      ...
      const expr = a.value.replace(/[{}]/g, "").trim();
      dyn.push(
        `bindClass(__el, ${JSON.stringify(mod)}, () => ${rw(expr, state, derived)});`,
      );
    } else if (a.name.startsWith("style:")) {
      const prop = a.name.slice(6);
      const expr = a.value.replace(/[{}]/g, "").trim();
      dyn.push(
        `bindStyle(__el, ${JSON.stringify(prop)}, () => ${rw(expr, state, derived)});`,
      );
    } else if (a.dynamic) {
      props.push(
        `${JSON.stringify(a.name)}: ${cliAttrValue(a.value, state, derived)}`,
      );
    } else {
  ...
  if (_scope)
    props.push(
      `${JSON.stringify("data-fly-scope")}: ${JSON.stringify(_scope)}`,
    );
...
export function generateActionClient(
...
function inlineClientEnv(code: string, env: Record<string, string>): string {
  code = code.replace(
    /(?:(?:import\.meta\.env)|(?:process\.env))\.(PUBLIC_[A-Za-z0-9_]+)/g,
    (_m, k) => JSON.stringify(env[k] ?? ""),
  );
  code = code.replace(
    /(?:(?:import\.meta\.env)|(?:process\.env))\.[A-Za-z0-9_]+/g,
    "undefined",
  );
  return code;
}
...
export function generateClient(
  ...
  const register = state
    .map(
      (n) => `try { __flyRegisterState(${JSON.stringify(n)}, ${n}); } catch {}`,
    )
    .join("\n");
  const raw =
    `import { signal, effect, derived, h, text, hIf, hFor, bindClass, bindStyle, bindValue, bindTransition, mountRoot, mountIsland, mountSuspense, mountAwait, dispatchIslands, onMount, onDestroy, writable, readable, get, tick, moveItem, draggable, dropTarget, bindDragReorder, __flyRegisterState } from "/__fly/client.js";\n` +
  ...
  while (i < value.length) {
    if (value[i] === "{") {
      ...
      for (; j < value.length; j++) {
        const c = value[j];
        if (inStr) {
          if (c === inStr && value[j - 1] !== "\\") inStr = null;
