...
function startApp(isDev: boolean) {
  ...
  if (adapter === "edge") {
    ...
    console.log(
      "Fly: adapter=edge (handler exportado). Servindo local via Node.",
    );
  } else if (adapter === "serverless") {
    createServerlessHandler(config as any);
    console.log(
      "Fly: adapter=serverless (handler exportado). Servindo local via Node.",
    );
  }
  startNodeServer({ ...(config as any), dev: isDev }, port);
}
...
async function buildCommand() {
  ...
  const b = createBuild(scan, {
    ...
    css: (config as any).css,
  });
  await b.buildProd();
  console.log(
    `Build ok: ${n} arquivos compilados -> .fly-out (SSG + client + SEO)`,
  );
}
...
function scaffold(name: string) {
  ...
  writeFileSync(
    join(dir, "layout.fly"),
    `<html lang="pt-BR"><head>__FLY_HEAD__</head><body><div id="__fly"><slot/></div>__FLY_SCRIPTS__</body></html>\n`,
  );
  writeFileSync(
    join(dir, "index.fly"),
  ...
  writeFileSync(
    join(dir, "api", "hello.fly"),
    `// API route\n\nexport function GET({ params }) {\n  return { message: "Olá do servidor!" }\n}\n`,
  );
  writeFileSync(
    join(dir, "actions", "todo.fly"),
...
if (cmd === "dev") startApp(true);
else if (cmd === "start") startApp(false);
else if (cmd === "build")
  buildCommand().catch((e: any) => {
    console.error(e);
    process.exit(1);
  });
  ...
  console.log(
    "Uso: fly <dev|build|start|check|create [nome]|add [rota]|init [dir]>",
  );
  console.log("  dev    sobe com HMR (preserva estado) + SSE em /__fly/hmr");
  console.log(
    "  build  SSG + CSS (tailwind standalone se css.tailwind) + tipos",
  );
  console.log("  check  só compila as rotas");
  process.exit(1);
}
